# Determine time to diagnosis for each case.
...
# List all phenotypes to analyze; Can be generated from Table S1 (column 1)
pheno_list <- as.data.frame(fread("/data/phenotypes/ICD10_binary_traits_definition.txt"))$phenotype

# Determine the disease to analyze
p <- pheno_list[p]
rm(pheno_list)

# Specify the analyzed sex depending on the disease
sex <- "All"
if (p %in% "PC") {sex <- "M"}
if (p %in% c("BC", "OC", "menstruation", "endometriosis")) {sex <- "F"}
print(paste0("Analyzing: ", p, " (in ", sex, ")"))
...
# Load disease data and select relevant column as the indiactor (case vs ctrl); File generated by "04_phenotypes/01_ICD10_extraction.R"
...
# Load disease definition file; Can be generated from Table S1
pheno_definition <- as.data.frame(fread("/data/phenotypes/ICD10_binary_traits_definition.txt", header = T, select = c(1,2), col.names = c("PHENO", "CODES")))
...
# Match and merge ICD10 codes and age at diagnosis
...
fwrite(pheno, paste0("/cauwerx/gwas/04_phenotypes/data/time_to_event/time_to_event_", sex, ".", p, ".txt.gz"), col.names = T, row.names = F, quote = F, sep = "\t")
