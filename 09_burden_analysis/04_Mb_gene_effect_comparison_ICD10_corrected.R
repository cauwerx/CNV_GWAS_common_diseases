# Compare the CORRECTED Mb vs. Gene burden effects through simulation analyses (ICD10s).
...
# Burden effects; Files generated by "09_burden_analysis/02_CNV_burden_binary_corrected.R"
burden_DEL <- as.data.frame(fread("/cauwerx/gwas/09_burden_analysis/data/CNVR_correction/DEL_burden_analysis_CNVR_correction_binary.txt", select = c(1,2,7,9)))
burden_DUP <- as.data.frame(fread("/cauwerx/gwas/09_burden_analysis/data/CNVR_correction/DUP_burden_analysis_CNVR_correction_binary.txt", select = c(1,2,7,9)))
...
fwrite(results, "/cauwerx/gwas/09_burden_analysis/data/Mb_gene_effect_comparison/Mb_gene_effect_comparison_ICD10s_corrected.txt", col.names = T, row.names = F, sep = "\t", quote = F)
