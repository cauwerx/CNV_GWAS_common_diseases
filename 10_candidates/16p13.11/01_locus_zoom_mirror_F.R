# Locus zoom plot for the association between the 16p13.11 locus and age at menarche according to the mirror association model (females-only).
...
# Output file
output_file <- paste0("/cauwerx/gwas/10_candidates/16p13.11/data/temp/locus_zoom/mirror_F_chr", chr, ":", start, ":", end)
...
for(p in unlist(strsplit(pheno, " "))) {
	...
	# Save
	fwrite(df, paste0("/cauwerx/gwas/10_candidates/16p13.11/data/locus_zoom/mirror_F_chr", chr, ":", start, "-", end, ".", p, ".glm.linear"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
