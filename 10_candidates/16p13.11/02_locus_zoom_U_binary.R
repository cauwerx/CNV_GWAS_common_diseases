# Locus zoom plot for the association between the 16p13.11 locus and HTN according to a U-shape association model.
...
# Output file
output_file <- paste0("/cauwerx/gwas/10_candidates/16p13.11/data/temp/locus_zoom/U_All_chr", chr, ":", start, ":", end)
...
for(p in unlist(strsplit(pheno, " "))) {
	...
	# Save
	fwrite(df, paste0("/cauwerx/gwas/10_candidates/16p13.11/data/locus_zoom/U_All_chr", chr, ":", start, ":", end, ".", p, ".glm.logistic.hybrid"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
