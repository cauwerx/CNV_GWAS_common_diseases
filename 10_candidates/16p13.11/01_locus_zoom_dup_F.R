# Locus zoom plot for the association between the 16p13.11 locus and agge at menpause according to the duplication-only association model (females nly).
...
pheno_file <- "/data/continuous_CNV_GWAS/pheno_continuous_WB_INT_age_age2_batch_PCs_F.txt"

# Samples: white-british individuals; Generated by "01_samples/sample_filtering.R"
samples_file <- "/cauwerx/gwas/01_samples/data/samples_white_british_F.txt"
...
# Output file
output_file <- paste0("/cauwerx/gwas/10_candidates/16p13.11/data/temp/locus_zoom/dup_F_chr", chr, ":", start, ":", end)
...
for(p in unlist(strsplit(pheno, " "))) {
	...
	# Save
	fwrite(df, paste0("/cauwerx/gwas/10_candidates/16p13.11/data/locus_zoom/dup_F_chr", chr, ":", start, "-", end, ".", p, ".glm.linear"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
