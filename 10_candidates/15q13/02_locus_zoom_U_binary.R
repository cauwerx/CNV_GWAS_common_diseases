# Locus zoom plot for the association between the 15q13 locus and hemorrhagic stroke according to a U-shaped association model.
...
# Output file
output_file <- paste0("/cauwerx/gwas/10_candidates/15q13/data/temp/locus_zoom/U_All_chr", chr, ":", start, ":", end)
...
for(p in unlist(strsplit(pheno, " "))) {
	...
	# Save
	fwrite(df, paste0("/cauwerx/gwas/10_candidates/15q13/data/locus_zoom/U_All_chr", chr, ":", start, ":", end, ".", p, ".glm.logistic.hybrid"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
