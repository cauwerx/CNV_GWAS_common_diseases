# Locus zoom plot for the association between the 15q13 locus and MCH and heart rate according to the duplication-only association model.
...
for(p in unlist(strsplit(pheno, " "))) {
	...
	# Save
	fwrite(df, paste0("/cauwerx/gwas/10_candidates/15q13/data/locus_zoom/dup_All_chr", chr, ":", start, "-", end, ".", p, ".glm.linear"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
