# Locus zoom plot for the association between the 16p12.2 locus and cardiopulmonary traits according to a deletion-only association model.
...
fwrite(data.frame(hm$SNP), "/cauwerx/gwas/10_candidates/16p12.2/data/temp/locus_zoom/high_missingness.txt", col.names = F, row.names = F, quote = F, sep = "\t")
...
HM_file <- "/cauwerx/gwas/10_candidates/16p12.2/data/temp/locus_zoom/high_missingness.txt"

# Output file
output_file <- paste0("/cauwerx/gwas/10_candidates/16p12.2/data/temp/locus_zoom/del_All_chr", chr, ":", start, ":", end)
...
for(p in unlist(strsplit(pheno, " "))) {
	...
	cov_file <- paste0("/cauwerx/gwas/10_candidates/16p12.2/data/temp/covariates/selected_covariates_All_", p, ".txt")
	...
	# Save
	fwrite(df, paste0("/cauwerx/gwas/10_candidates/16p12.2/data/locus_zoom/del_All_chr", chr, ":", start, ":", end, ".", p, ".glm.logistic.hybrid"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
