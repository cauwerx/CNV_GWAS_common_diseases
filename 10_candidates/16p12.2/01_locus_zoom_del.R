# Locus zoom plot for the association between the 16p12.2 locus and cardiopulmonary disease according to the deletion-only association model.
...
for(p in unlist(strsplit(pheno, " "))) {
	...
	# Save
	fwrite(df, paste0("/cauwerx/gwas/10_candidates/16p12.2/data/locus_zoom/del_All_chr", chr, ":", start, "-", end, ".", p, ".glm.linear"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
