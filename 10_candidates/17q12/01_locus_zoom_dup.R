# Locus zoom plot for the association between the RCAD locus and kindney biomarkers according to a duplication-only association model.
...
plink_file <- paste0("/data/UKBB/cnv_calls/PLINK/duplication_only/ukb_cnv_bed/ukb_cnv_chr", chr)
...
# Output file
output_file <- paste0("/cauwerx/gwas/10_candidates/17q12/data/temp/locus_zoom/dup_All_chr", chr, ":", start, ":", end)
...
for(p in unlist(strsplit(pheno, " "))) {
	...
	# Save
	fwrite(df, paste0("/cauwerx/gwas/10_candidates/17q12/data/locus_zoom/dup_All_chr", chr, ":", start, "-", end, ".", p, ".glm.linear"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
