# Locus zoom plot for the association between the RCAD locus and CKD according to a mirror association model.
...
fwrite(data.frame(hm$SNP), "/cauwerx/gwas/10_candidates/17q12/data/temp/locus_zoom/high_missingness.txt", col.names = F, row.names = F, quote = F, sep = "\t")
...
# Covariate file (select relevant covariates for CKD)
selected_cov <- as.data.frame(fread(paste0("/cauwerx/gwas/05_gwas/02_covariate_selection/data/All/covariates_All.", pheno, ".txt"), header = F, sep = "\t")) # File generated by "05_gwas/02_covariate_selection/01_covariate_selection.R"
...
cov_file <- paste0("/cauwerx/gwas/10_candidates/17q12/data/temp/covariates/selected_covariates_All_", pheno, ".txt")
...
HM_file <- "/cauwerx/gwas/10_candidates/17q12/data/temp/locus_zoom/high_missingness.txt"

# Output file
output_file <- paste0("/cauwerx/gwas/10_candidates/17q12/data/temp/locus_zoom/U_All_chr", chr, ":", start, ":", end)
...
system(paste("/cauwerx/softwares/plink2/plink2",
			 "--threads 20",
 			 "--bfile", plink_file,
			 "--pheno iid-only", pheno_file, "--no-psam-pheno", "--pheno-name", pheno,
...
# Load file
df <- as.data.frame(fread(paste0(output_file, ".", pheno, ".glm.logistic.hybrid"), header = T))
...
# Save
fwrite(df, paste0("/cauwerx/gwas/10_candidates/17q12/data/locus_zoom/U_All_chr", chr, ":", start, ":", end, ".", pheno, ".glm.logistic.hybrid"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
