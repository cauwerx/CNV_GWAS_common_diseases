# Determine time to hypercholesterolemia diagnosis among LDLR deletion carriers.
...
pheno_list <- as.data.frame(fread("/data/phenotypes/ICD10_hypercholesterolemia.txt"))$phenotype
p <- pheno_list[p]
rm(pheno_list)
...
# Load disease data and select relevant column as the indiactor (case vs ctrl); File generated by "10_candidates/19p13.2/01_ICD10_extraction.R"
pheno <- as.data.frame(fread("/cauwerx/gwas/10_candidates/19p13.2/data/pheno/hypercholesterolemia_ICD10_All.txt"))
...
# Phenotype definitions (inclusion and exclusion lists for hypercholesterolemia with and without exclusion of people with related conditions)
pheno_definition <- as.data.frame(fread("/data/phenotypes/ICD10_hypercholesterolemia.txt", header = T, select = c(1,2), col.names = c("PHENO", "CODES")))
...
fwrite(pheno, paste0("/cauwerx/gwas/10_candidates/19p13.2/data/time_to_event/time_to_event_All.", p, ".txt.gz"), col.names = T, row.names = F, quote = F, sep = "\t")
