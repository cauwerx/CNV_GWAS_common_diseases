# Determine time to female reproductive cancer diagnosis among BRCA1 deletion carriers. 
...
pheno_list <- as.data.frame(fread("/data/phenotypes/ICD10_female_cancer.txt"))$phenotype
p <- pheno_list[p]
rm(pheno_list)
...
# Load disease data and select relevant column as the indiactor (case vs ctrl); File generated by "10_candidates/17q21.31/01_ICD10_extraction.R"
pheno <- as.data.frame(fread("/cauwerx/gwas/10_candidates/17q21.31/data/pheno/female_cancer_ICD10_F.txt"))
...
# Phenotype definitions (inclusion and exclusion lists for female reproductive cancers with and without exclusion of people with related conditions)
pheno_definition <- as.data.frame(fread("/data/phenotypes/ICD10_female_cancer.txt", header = T, select = c(1,2), col.names = c("PHENO", "CODES")))
...
fwrite(pheno, paste0("/cauwerx/gwas/10_candidates/17q21.31/data/time_to_event/time_to_event_F.", p, ".txt.gz"), col.names = T, row.names = F, quote = F, sep = "\t")
