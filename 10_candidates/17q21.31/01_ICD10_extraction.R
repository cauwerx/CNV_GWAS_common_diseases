# Extract phenotype information for female breast/reproductive cancers and generate a PLINK-compatible file
...
# Phenotype list; Contains the definition of inclusion and exclusion lists for female breast/reproductive cancer with and without exclusion of people with other cancers
pheno_list <- as.data.frame(fread("/data/phenotypes/ICD10_female_cancer.txt", header = T))
...
#################################################
### STEP 4: Split by sex and clean ##############

# Subset female individuals
eid_F <- as.data.frame(fread("/cauwerx/gwas/01_samples/data/samples_white_british_F.txt", header = F, col.names = "eid"))
...
fwrite(pheno_F, "/cauwerx/gwas/10_candidates/17q21.31/data/pheno/female_cancer_ICD10_F.txt", col.names = T, row.names = F, quote = F, sep = "\t", na = "NA")
