# Calculate the CNV region (CNVR) for each GWAS hit according to the deletion-only model.
...
# Load the last summary file from the SCA; File generated by "06_SCA/deletion_only/SCA_binary_del.R"
files <- mixedsort(list.files(paste0("/cauwerx/gwas/06_SCA/deletion_only/data/temp/summary/", sex), recursive = F, full.names = T))
...
for(i in 1:nrow(unique_combo)) { 
	...
	rs_file <- paste0("/cauwerx/gwas/07_CNVR/deletion_only/data/temp/", sex, "/", rs)
...
fwrite(df, paste0("/cauwerx/gwas/07_CNVR/deletion_only/data/GWAS_CNVR_", sex,"_del.txt"), col.names = T, row.names = F, quote = F, sep = "\t")
