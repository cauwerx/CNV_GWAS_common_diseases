# Calculate the CNV region (CNVR) for each GWAS hit according to the mirror model.
...
files <- mixedsort(list.files(paste0("/home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/08_SCA/binary/white_british/mirror/data/temp/summary/", sex), recursive = F, full.names = T))
...
for(i in 1:nrow(unique_combo)) {
	...
	rs_file <- paste0("/cauwerx/gwas/07_CNVR/mirror/data/temp/", sex, "/", rs)
...
fwrite(df, paste0("/cauwerx/gwas/07_CNVR/mirror/data/GWAS_CNVR_", sex,"_mirror.txt"), col.names = T, row.names = F, quote = F, sep = "\t")
