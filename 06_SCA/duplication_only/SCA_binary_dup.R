# Stepwise conditional analysis. Configuration: Duplication-only.
...
# Number of significant associations; File generated by "05_gwas/05_pruning/duplication_only/01_pruning_dup.R" 
n <- nrow(as.data.frame(fread(paste0("/cauwerx/gwas/05_gwas/05_pruning/duplication_only/data/", sex, "/00_qc_probes_gwas_dup_", sex, ".txt"), header = T)))
...
if (round == 1) {
		# At first round, file generated by "05_gwas/05_pruning/duplication_only/01_pruning_dup.R"
		new_sig <- as.data.frame(fread(paste0("/cauwerx/gwas/05_gwas/05_pruning/duplication_only/data/", sex, "/00_qc_probes_gwas_dup_", sex, ".txt"), header = T))
	} else {
		# At next rounds, file generated by this script	
		new_sig <- as.data.frame(fread(paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/significant_associations/", sex, "/sig_assoc_binary_dup_SCA_", sex, "_round", round-1, ".txt.gz"), header = T))}
...
if (round == 1) {
		...
		sum_file <- as.data.frame(fread(paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/summary/", sex, "/summary_dup_SCA_", sex, "_round", round-1, ".txt"), header = T))
		sum_file <- rbind(sum_file, new_sig)}

print(paste0("Total number of independent signals after round ",round,": ", nrow(sum_file)))
fwrite(sum_file, paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/summary/", sex, "/summary_dup_SCA_", sex, "_round", round, ".txt"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
...
for(i in 1:nrow(probes)) {
	...
	# Write file
	fwrite(data.frame(rs), paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/cnv_profiles/", sex, "/", rs), col.names = F, row.names = F, quote = F, sep = "\t")
	...
	output <- paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/cnv_profiles/", sex, "/", rs)
...
rm(i, rs, chr, input, output, ped, samples)
...
for (p in new_sig$PHENO) {
	...
	if (round == 1) {
			...
			if (file.size(paste0("/cauwerx/gwas/05_gwas/02_covariate_selection/data/", sex, "/covariates_", sex, ".", p,".txt")) > 1) {
				selected_cov <- as.data.frame(fread(paste0("/cauwerx/gwas/06_gwas/02_covariate_selection/data/", sex, "/covariates_", sex, ".", p,".txt"), header = F, sep = "\t"))
			...
			cov <- as.data.frame(fread(paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/covariates/", sex, "/covariates_",  sex, "_round", round-1, ".", p,".txt"), header = T))}

	# Add the profile of the top probe to the covariate file
	new_cov_profile <- as.data.frame(fread(paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/cnv_profiles/", sex, "/", new_cov,"_profile"), header = T))
	cov <- right_join(cov, new_cov_profile, by = "IID")

	# Save the updated covariate file & delete the old one
	fwrite(cov, paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/covariates/", sex, "/covariates_", sex ,"_round", round, ".", p,".txt"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
	if(round != 1){ 
	unlink(paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/covariates/", sex, "/covariates_", sex, "_round", round-1, ".", p,".txt"))}
...
system(paste0("mkdir /home/cauwerx/scratch/cauwerx/projects/cnv_gwas/gwas/08_SCA/binary/white_british/duplication_only/data/temp/associations/", sex, "/round", round))
...
for (p in new_sig$PHENO) {
	...
	for(chr in chromosomes) {
		...
		covar_file <- paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/covariates/", sex, "/covariates_", sex, "_round", round, ".", p,".txt")	
		...
		probe_file <- paste0("/cauwerx/gwas/05_gwas/05_pruning/duplication_only/data/", sex, "/SCA_probes/qc_probes_dup_", sex, ".", p,".txt") # File generated by "05_gwas/05_pruing/duplication_only/01_pruning_dup.R"
		output <- paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/associations/", sex, "/round", round,"/gwas_binary_dup_SCA_", sex, "_round", round,"_chr", chr)
...
list_gwas <- mixedsort(list.files(paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/associations/", sex, "/round", round), pattern = ".glm", full.names = T, recursive = F))
...
fwrite(sig_gwas, paste0("/cauwerx/gwas/06_SCA/duplication_only/data/temp/significant_associations/", sex, "/sig_assoc_binary_dup_SCA_", sex, "_round", round, ".txt.gz"), col.names = T, row.names = F, quote = F, sep = "\t", na = NA)
rm(f, df, temp_sig)
