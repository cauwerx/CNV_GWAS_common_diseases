# Prune significant probes from the CNV-GWAS. Configuration: Mirror model.
...
# Load significant GWAS data; File generated by "05_gwas/04_gwas/mirror/02_correct_merge_binary_gwas_mirror.R" 
sig_probes <- as.data.frame(fread(paste0("/cauwerx/gwas/05_gwas/04_gwas/mirror/data/", sex, "/significant/00_significant_mirror_", sex, ".txt")))
...
for (i in 1:nrow(unique_combo)){
	...
	# Define disease and chromosome & selecct probes
	...
	# File names
	result_file <- paste0("/cauwerx/gwas/05_gwas/05_pruning/mirror/data/temp/", sex, "/probes_mirror_", sex, "_r2_", r2, "_chr", chr, ".", p)
	probe_file <- paste0("/cauwerx/gwas/05_gwas/05_pruning/mirror/data/temp/", sex, "/probes/input_probes_mirror_", sex, "_chr", chr,".", p, ".txt")
	fake_freq_file <- paste0("/cauwerx/gwas/05_gwas/05_pruning/mirror/data/temp/", sex, "/p_value_freq/p_val_freq_mirror_", sex, "_chr", chr, ".", p, ".afreq")
...
fwrite(summary_pruning, paste0("/cauwerx/gwas/05_gwas/05_pruning/mirror/data/", sex, "/00_summary_pruning_mirror_", sex, ".txt"), row.names = F, col.names = T, quote = F, sep = "\t") 
...
for (up in unique_p){
	...
	prune.in_list <- list.files(path = paste0("/cauwerx/gwas/05_gwas/05_pruning/mirror/data/temp/", sex), pattern = paste0("\\.", up, ".prune.in"), full.names = T, recursive = F)
	...
	# Load relevant GWAS association data; File generated by "05_gwas/04_gwas/mirror/02_correct_merge_binary_gwas_mirror.R" 
	df_gwas <- as.data.frame(fread(paste0("/cauwerx/gwas/05_gwas/04_gwas/mirror/data/", sex, "/significant/gwas_significant_mirror_", sex, ".", up, ".txt")))
	...
	fwrite(data.frame(ID = df_gwas_probes$ID), paste0("/cauwerx/gwas/05_gwas/05_pruning/mirror/data/", sex, "/SCA_probes/qc_probes_mirror_", sex, ".", up, ".txt"), col.names = F, row.names = F, quote = F, sep = "\t")
...
fwrite(summary_qc, paste0("/cauwerx/gwas/05_gwas/05_pruning/mirror/data/", sex, "/00_qc_probes_gwas_mirror_", sex, ".txt"), col.names = T, row.names = F, quote = F, sep = "\t")
