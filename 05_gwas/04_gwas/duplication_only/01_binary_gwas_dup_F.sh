...
#SBATCH --job-name=gwas_dup_F      	# Give your job a name to recognize it in queue overview
...
#SBATCH --array=1-24 
#SBATCH --output=/cauwerx/gwas/05_gwas/04_gwas/duplication_only/data/log/01_gwas_dup_F-%A_%a.out
...
pheno_all=$(echo "/cauwerx/gwas/04_phenotypes/data/pheno_ICD10_F.txt")
...
# Output
output_F=$(echo "/cauwerx/gwas/05_gwas/04_gwas/duplication_only/data/temp/F/gwas_dup_F_chr${chr}")
...
for p in $pheno_list; do
	...
	selected_probes=$(echo "/cauwerx/gwas/05_gwas/03_probe_selection/data/F/duplication_only/probes_dup_F_chr${chr}.${p}.txt")
