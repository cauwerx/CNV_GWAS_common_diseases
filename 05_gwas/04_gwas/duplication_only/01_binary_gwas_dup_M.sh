...
#SBATCH --job-name=gwas_dup_M      	# Give your job a name to recognize it in queue overview
...
#SBATCH --array=1-24 
#SBATCH --output=/cauwerx/gwas/05_gwas/04_gwas/duplication_only/data/log/01_gwas_dup_M-%A_%a.out
...
pheno_all=$(echo "/cauwerx/gwas/04_phenotypes/data/pheno_ICD10_M.txt")
...
# Output
output_M=$(echo "/cauwerx/gwas/05_gwas/04_gwas/duplication_only/data/temp/M/gwas_dup_M_chr${chr}")
...
for p in $pheno_list; do
	...
	selected_probes=$(echo "/cauwerx/gwas/05_gwas/03_probe_selection/data/M/duplication_only/probes_dup_M_chr${chr}.${p}.txt")
