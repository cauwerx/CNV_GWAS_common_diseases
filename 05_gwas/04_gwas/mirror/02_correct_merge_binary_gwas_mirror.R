# Correct the OR, merge files by disease, select significant associations. Configuration: mirror.
...
# List all files to correct; Generated by "05_gwas/04_gwas/01_binary_gwas_mirror_*.sh"
files_to_correct <- list.files(paste0("/cauwerx/gwas/05_gwas/04_gwas/mirror/data/temp/", sex), pattern = "*.glm.*", recursive = F, full.names = T)
...
for (p in all_pheno) {
	...
	# Define the disease
	print(paste0("Analyzing: ", p, " (mirror, ", sex,")"))

	# List files corresponding to the defined phenotype; Generated by "05_gwas/04_gwas/01_binary_gwas_mirror_*.sh"
	files_p <- mixedsort(list.files(paste0("/cauwerx/gwas/05_gwas/04_gwas/mirror/data/temp/", sex), pattern = paste0("*\\.", p, ".glm"), recursive = F, full.names = T))
	...
	# Save
	fwrite(df, paste0("/cauwerx/gwas/05_gwas/04_gwas/mirror/data/", sex,"/gwas_mirror_", sex,".", p, ".txt"), col.names = T, row.names = F, sep = "\t", quote = F)

	# Retain significant files & Save
	sig_df <- df[df$P <= thr & !is.na(df$P), ]
	print(paste0("Number of significant associations: ", nrow(sig_df)))
	if (nrow(sig_df) > 0) {
		fwrite(sig_df, paste0("/cauwerx/gwas/05_gwas/04_gwas/mirror/data/", sex,"/significant/gwas_significant_mirror_", sex,".", p, ".txt"), col.names = T, row.names = F, sep = "\t", quote = F)}
...
fwrite(significant_associations, paste0("/cauwerx/gwas/05_gwas/04_gwas/mirror/data/", sex,"/significant/00_significant_mirror_", sex,".txt"), col.names = T, row.names = F, sep = "\t", quote = F)
