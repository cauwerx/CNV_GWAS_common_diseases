...
#SBATCH --job-name=gwas_U_F      	# Give your job a name to recognize it in queue overview
...
#SBATCH --array=1-24 
#SBATCH --output=/cauwerx/gwas/05_gwas/04_gwas/u_shape/data/log/01_gwas_U_F-%A_%a.out
...
pheno_F=$(echo "/cauwerx/gwas/05_phenotypes/data/pheno_ICD10_F.txt")
...
# Output
output_F=$(echo "/cauwerx/gwas/05_gwas/04_gwas/u_shape/data/temp/F/gwas_U_F_chr${chr}")
...
for p in $pheno_list; do
	...
	selected_covariates=$(cat "/cauwerx/gwas/05_gwas02_covariate_selection/data/F/covariates_F.${p}.txt")

	# Define selected probes; File generated by "05_gwas/03_probe_selection/02_probe_selection.R"
	selected_probes=$(echo "/cauwerx/gwas/05_gwas/03_probe_selection/data/F/mirror/probes_mirror_F_chr${chr}.${p}.txt")
	...
	then
		...
		/cauwerx/softwares/plink2/plink2 	--threads 20 \
											...
											--pheno iid-only ${pheno_F} --no-psam-pheno --pheno-name ${p} \
			 								--glm firth-fallback omit-ref no-x-sex hide-covar allow-no-covars hetonly --ci 0.95 \
											--out ${output_F}
	else
