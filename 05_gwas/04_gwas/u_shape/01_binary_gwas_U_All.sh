...
#SBATCH --job-name=gwas_U_All      	# Give your job a name to recognize it in queue overview
...
#SBATCH --array=1-24 
#SBATCH --output=/cauwerx/gwas/05_gwas/04_gwas/u_shape/data/log/01_gwas_U_All-%A_%a.out
...
pheno_all=$(echo "/cauwerx/pgwas/04_phenotypes/data/pheno_ICD10_All.txt")
...
# Output
output_all=$(echo "/cauwerx/gwas/05_gwas/04_gwas/u_shape/data/temp/All/gwas_U_All_chr${chr}")
...
for p in $pheno_list; do
	...
	selected_probes=$(echo "/cauwerx/gwas/05_gwas/03_probe_selection/data/All/mirror/probes_mirror_All_chr${chr}.${p}.txt")
