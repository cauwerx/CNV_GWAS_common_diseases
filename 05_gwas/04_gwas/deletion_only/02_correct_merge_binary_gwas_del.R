# Correct the OR, merge files by disease, and select significant associations. Configuration: deletion-only.
...
# List all files to correct; Generated by "05_gwas/04_gwas/01_binary_gwas_del_*.sh"
files_to_correct <- list.files(paste0("/cauwerx/gwas/05_gwas/04_gwas/deletion_only/data/temp/", sex), pattern = "*.glm.*", recursive = F, full.names = T)
...
for (p in all_pheno) {
	...
	# Define the disease
	print(paste0("Analyzing: ", p, " (deletion-only, ", sex,")"))

	# List files corresponding to the defined phenotype; Generated by "05_gwas/04_gwas/01_binary_gwas_del_*.sh"
	files_p <- mixedsort(list.files(paste0("/cauwerx/gwas/05_gwas/04_gwas/deletion_only/data/temp/", sex), pattern = paste0("*\\.", p, ".glm"), recursive = F, full.names = T))
	...
	# Save
	fwrite(df, paste0("/cauwerx/gwas/05_gwas/04_gwas/deletion_only/data/", sex,"/gwas_del_", sex,".", p, ".txt"), col.names = T, row.names = F, sep = "\t", quote = F)

	# Retain significant files & save
	sig_df <- df[df$P <= thr & !is.na(df$P), ]
	print(paste0("Number of significant associations: ", nrow(sig_df)))
	if (nrow(sig_df) > 0) {
		fwrite(sig_df, paste0("/cauwerx/gwas/05_gwas/04_gwas/deletion_only/data/", sex,"/significant/gwas_significant_del_", sex,".", p, ".txt"), col.names = T, row.names = F, sep = "\t", quote = F)}
...
fwrite(significant_associations, paste0("/cauwerx/gwas/05_gwas/04_gwas/deletion_only/data/", sex,"/significant/00_significant_del_", sex,".txt"), col.names = T, row.names = F, sep = "\t", quote = F)
